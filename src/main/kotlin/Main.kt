...
open class Funcionario(var nome: String, var idade: Int) {
    ...
    fun apresentar() {
        println("Oi! Meu nome é ${nome} e tenho ${idade} anos.")
    }
}
...
fun main() {
    val listaFuncionarios: List<Funcionario> = listOf(
        Gerente("Juan", 18, "Desenvolvimento"),
        Gerente("Farias Adocicado", 44, "Relações Interpessoais"),
        Desenvolvedor("Samuel", 22, "TS"),
        Desenvolvedor("Zuleide", 41, "PHP"),
        Analista("Juquinha", 20, "QA"),
        Analista("Jurema", 26, "Seg. Dados")))

    listaFuncionarios.forEach {
        if (it is Gerente) {
            val gerente = it as Gerente
            print("Gerente do setor:  ${gerente.setor} -> ")
            gerente.apresentar()
        } else if (it is Desenvolvedor) {
            val desenvolvedor = it as Desenvolvedor
            print("Desenvolvedor que usa a linguagem: ${desenvolvedor.linguagem} -> ")
            desenvolvedor.apresentar()
        } else if (it is Analista) {
            val analista = it as Analista
            print("Analista da área:  ${analista.area} ---> ")
            analista.apresentar()
        }
    }
}
